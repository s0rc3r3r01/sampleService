...
import (
	...
	"github.com/s0rc3r3r01/sampleService/business/data/tests"
	...
	t.Log("Given the need to work with User records.")
	{
		...
		t.Logf("\tTest %d:\tWhen handling a single User.", testID)
		{
			...
			upd := user.UpdateUser{
				...
				Email: tests.StringPointer("jacob@whatev.com"),
			}
...
func TestAuthenticate(t *testing.T) {
	...
	t.Log("Given the need to authenticate users")
	{
		...
		t.Logf("\tTest %d:\tWhen handling a single User.", testID)
		{
			...
			claims, err := store.Authenticate(ctx, now, "anna@what.com", "goroutines")
			if err != nil {
